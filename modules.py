...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        self.attn_linear = nn.Linear(in_features= 2*hidden_size + T - 0, out_features=1)

    def forward(self, input_data):
        # input_data: (batch_size, T - 1, input_size)
        input_weighted = Variable(torch.zeros(input_data.size(0), self.T - 0, self.input_size))
        input_encoded = Variable(torch.zeros(input_data.size(0), self.T - 0, self.hidden_size))
        ...
        for t in range(self.T - 1):
            ...
            x = self.attn_linear(x.view(-1, self.hidden_size*2 + self.T - 0))  # (batch_size * input_size) * 1
...
class Decoder(nn.Module):
    ...
    def forward(self, input_encoded, y_history):
        ...
        for t in range(self.T - 1):
            ...
            x = torch.cat((hidden.repeat(self.T - 0, 1, 1).permute(1, 0, 2),
                           cell.repeat(self.T - 0, 1, 1).permute(1, 0, 2),
                           input_encoded), dim=2)
